...
class DeterministicEncoder(layers.Layer):
    def __init__(self, hidden_sizes=[128, 128, 128, 128], use_batch_norm=True, **kwargs):
        ...
        for size in hidden_sizes[:-1]:
            self.hidden_layers.append(layers.Dense(size, activation="gelu"))  # GELU is more efficient
    ...
    def build(self, input_shape):
        ...
        for i, layer in enumerate(self.hidden_layers):
            ...
            if self.use_batch_norm and i < len(self.batch_norm_layers):
                self.batch_norm_layers[i].build(input_shape=(None, None, layer.units))
            current_shape_dim = (
                layer.units
    ...
    def call(self, context_x, context_y, training=False):
        # Concatenate x and y along feature dimension
        encoder_input = ops.concatenate([context_x, context_y], axis=-1)

        # Pass through optimized MLP with batch normalization
        hidden = encoder_input
        for i, layer in enumerate(self.hidden_layers):
            hidden = layer(hidden)
            if self.use_batch_norm and i < len(self.batch_norm_layers):
                hidden = self.batch_norm_layers[i](hidden, training=training)

        # Average over context points (more efficient than mean with keepdims)
...
class AttentiveEncoder(layers.Layer):
    def __init__(self, encoder_sizes=[128, 128, 128, 128], num_heads=8, dropout_rate=0.1, **kwargs):
        super().__init__(**kwargs)
        ...
        # 2. Self-Attention over context points with optimized settings
        self.self_attention = layers.MultiHeadAttention(
            num_heads=self.num_heads, 
            key_dim=self.rep_dim // self.num_heads,  # More efficient key dimension
            dropout=self.dropout_rate
        )
        self.self_attention_norm = layers.LayerNormalization()
        self.self_attention_dropout = layers.Dropout(self.dropout_rate)
        ...
        self.cross_attention = layers.MultiHeadAttention(
            num_heads=self.num_heads, 
            key_dim=self.rep_dim // self.num_heads,
            dropout=self.dropout_rate
        )
        self.cross_attention_norm = layers.LayerNormalization()
        self.cross_attention_dropout = layers.Dropout(self.dropout_rate)
    ...
    def build(self, input_shape):
        ...
        self.self_attention_dropout.build(attention_input_shape)
        ...
        self.cross_attention_dropout.build(query_shape)

        super().build(input_shape)

    def call(self, context_x, context_y, target_x, training=False):
        # Efficient processing of context points
        encoder_input = ops.concatenate([context_x, context_y], axis=-1)
        hidden = encoder_input
        for layer in self.context_mlp_hidden_layers:
            hidden = layer(hidden)
        ...
        # Optimized self-attention with residual connection and dropout
        ...
        self_att_output = self.self_attention_dropout(self_att_output, training=training)
        ...
        # Optimized cross-attention with residual connection and dropout
        ...
        cross_att_output = self.cross_attention_dropout(cross_att_output, training=training)
...
class DeterministicDecoder(layers.Layer):
    ...
    def call(self, representation, target_x):
        # Efficient representation expansion using broadcasting instead of repeat
        ...
        if rep_num_points == 1:
            # CNP case: use broadcasting for memory efficiency
            # Instead of ops.repeat which creates a copy, use broadcasting
            representation = ops.broadcast_to(
                representation, 
                (ops.shape(representation)[0], target_num_points, ops.shape(representation)[2])
            )
...
class CNP(keras.Model):
    ...
    def call(self, inputs, training=False):
        ...
        representation = self.encoder(
            context_x, context_y, training=training
...
class NP(keras.Model):
    ...
    def call(self, inputs, training=False):
        ...
        # --- Deterministic Path ---
        det_rep = self.det_encoder(context_x, context_y, training=training)  # Shape: (batch, 1, dim)
        det_rep = ops.repeat(
...
class ANP(keras.Model):
    ...
    def call(self, inputs, training=False):
        ...
        # --- Deterministic Path ---
        det_rep = self.att_encoder(context_x, context_y, target_x, training=training)
