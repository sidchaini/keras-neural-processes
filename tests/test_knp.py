...
@pytest.fixture
def sample_data_1d():
    """Generate sample 1D function data for testing."""
    batch_size = 4
    num_points = 50
    x_dim = 1
    y_dim = 1
...
def sample_data_multi_channel():
    """Generate sample multi-channel data for testing."""
    batch_size = 4
    num_points_per_channel = 25
    num_channels = 2
    total_points = num_points_per_channel * num_channels
    
    # Create multi-channel data in long format
    ...
    for batch_idx in range(batch_size):
        for ch in range(num_channels):
            ...
            x[batch_idx, start_idx:end_idx, 1] = ch  # channel_id
...
# Tests for basic model instantiation (existing tests)
def test_CNP():
    model = knp.CNP()
...
class TestLayers:
    ...
    def test_attentive_encoder(self, context_target_data):
        ...
        expected_shape = (target_x.shape[0], target_x.shape[1], 32)  # batch, num_targets, hidden_size
        assert representation.shape == expected_shape
    ...
    def test_deterministic_decoder(self, context_target_data):
        ...
        representation = np.random.randn(
            target_x.shape[0], target_x.shape[1], representation_dim  # Same num_points as target
        ).astype(np.float32)
...
class TestErrorHandling:
    ...
    def test_get_context_set_too_many_points(self, sample_data_1d):
        ...
        with pytest.raises(ValueError):
            utils.get_context_set(x, y, num_context=num_points + 10, num_context_mode="all")
